...
class GpuBuffer {
  ...
  public GpuBuffer(int target, int numberOfBytesPerEntry, Buffer entries) {
    ...
    try {
      ...
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to unbind vertex array", "glBindVertexArray");

      GLES30.glGenBuffers(1, bufferId, 0);
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to generate buffers", "glGenBuffers");

      GLES30.glBindBuffer(target, bufferId[0]);
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to bind buffer object", "glBindBuffer");
      ...
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to populate buffer object", "glBufferData");
    } catch (Throwable t) {
      free();
      throw t;
    }
  ...
  public void set(Buffer entries) {
    ...
    GLES30.glBindBuffer(target, bufferId[0]);
    com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to bind vertex buffer object", "glBindBuffer");

    entries.rewind();

    if (entries.limit() <= capacity) {
      GLES30.glBufferSubData(target, 0, entries.limit() * numberOfBytesPerEntry, entries);
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to populate vertex buffer object", "glBufferSubData");
      size = entries.limit();
    } else {
      GLES30.glBufferData(
          target, entries.limit() * numberOfBytesPerEntry, entries, GLES30.GL_DYNAMIC_DRAW);
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to populate vertex buffer object", "glBufferData");
      size = entries.limit();
  ...
  public void free() {
    if (bufferId[0] != 0) {
      ...
      com.ubc.common.samplerender.GLError.maybeLogGLError(Log.WARN, TAG, "Failed to free buffer object", "glDeleteBuffers");
      bufferId[0] = 0;
    }
  }
