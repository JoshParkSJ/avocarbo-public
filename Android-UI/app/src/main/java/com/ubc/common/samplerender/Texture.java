...
public class Texture implements Closeable {
  ...
  public Texture(SampleRender render, Target target, WrapMode wrapMode, boolean useMipmaps) {
    ...
    com.ubc.common.samplerender.GLError.maybeThrowGLException("Texture creation failed", "glGenTextures");

    int minFilter = useMipmaps ? GLES30.GL_LINEAR_MIPMAP_LINEAR : GLES30.GL_LINEAR;

    try {
      GLES30.glBindTexture(target.glesEnum, textureId[0]);
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to bind texture", "glBindTexture");
      GLES30.glTexParameteri(target.glesEnum, GLES30.GL_TEXTURE_MIN_FILTER, minFilter);
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to set texture parameter", "glTexParameteri");
  ...
  public static Texture createFromAsset(
    ...
    try {
      ...
      GLES30.glTexImage2D(
          ...
          buffer);
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to populate texture data", "glTexImage2D");
      GLES30.glGenerateMipmap(GLES30.GL_TEXTURE_2D);
      com.ubc.common.samplerender.GLError.maybeThrowGLException("Failed to generate mipmaps", "glGenerateMipmap");
    } catch (Throwable t) {
      texture.close();
      throw t;
    } finally {
  ...
  public void close() {
    if (textureId[0] != 0) {
      ...
      com.ubc.common.samplerender.GLError.maybeLogGLError(Log.WARN, TAG, "Failed to free texture", "glDeleteTextures");
      textureId[0] = 0;
    }
  }
